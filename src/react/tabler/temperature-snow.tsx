// This file is autogenerated by "make react_icons", do not edit it.
import * as React from 'react';
const TemperatureSnow = (props: React.SVGProps<SVGSVGElement>) => <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="icon icon-tabler icons-tabler-outline icon-tabler-temperature-snow" {...props}><path d="M0 0h24v24H0z" stroke="none"/><path d="M4 13.5a4 4 0 104 0V5a2 2 0 10-4 0v8.5M4 9h4M14.75 4l1 2H18"/><path d="M17 4l-3 5 2 3M20.25 10L19 12l1.25 2"/><path d="M22 12h-6l-2 3M18 18h-2.25l-1 2"/><path d="M17 20l-3-5h-1M12 9l2.088.008"/></svg>;
export default TemperatureSnow;
