// This file is autogenerated by "make react_icons", do not edit it.
import * as React from 'react';
const StackOverflowLogo = (props: React.SVGProps<SVGSVGElement>) => <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" fill="currentColor" {...props}><path d="M216 152.09V216a8 8 0 01-8 8H48a8 8 0 01-8-8v-63.91a8 8 0 0116 0V208h144v-55.91a8 8 0 0116 0zm-128 32h80a8 8 0 100-16H88a8 8 0 100 16zm4.88-53l77.27 20.68a7.89 7.89 0 002.08.28 8 8 0 002.07-15.71L97 115.61A8 8 0 1092.88 131zm18.45-49.93l69.28 40a8 8 0 0010.93-2.93 8 8 0 00-2.93-10.91l-69.28-40.05a8 8 0 10-8 13.84zm87.33 13A8 8 0 10210 82.84l-56.57-56.5a8 8 0 00-11.32 11.3z"/></svg>;
export default StackOverflowLogo;
